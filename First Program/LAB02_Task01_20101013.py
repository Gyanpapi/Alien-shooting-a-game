...
def mouse(button, state, x, y):
    ...
    if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
        if 20<=x <= 70 and y<=(60):  # Restart button
    ...
    if not game_paused:
        ...
        if diamond_y <=25:
            if diamond_x+16 >= catcher_x and diamond_x <= catcher_x + 100:    # +16 for diamond width and +100 for catcher width
                ...
                score += 1
                diamond_speed=min(diamond_speed+1,70)  # increasing diamond speed #min function to avoid too much speed
...
def reset_game():
    global score, game_over, game_paused,catcher_speed,diamond_speed
    catcher_speed=25
    diamond_speed=10
    score = 0
    game_over = False
