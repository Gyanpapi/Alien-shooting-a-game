...
left_button_pressed = False  # Flag to track left mouse button press
...
def keyboardListener(key, x, y):
    global ball_size, move_points, blinking
    if key == b'w':
        ball_size += 1
...
def mouseListener(button, state, x, y):
    global balls, ball_speed, a, b, c, left_button_pressed
    if button == GLUT_LEFT_BUTTON:
        if state == GLUT_DOWN:
            ...
            glClearColor(0, 0, 0, 0.0)  # Set the clear color to transparent (original state)
        glutPostRedisplay()
