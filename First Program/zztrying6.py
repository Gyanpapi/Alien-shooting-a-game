...
def display():
    ...
    draw_buttons()
    if game_over:
        draw_text(width / 2.5, height / 2, "GAME OVER")
...
def keyboard(key, x, y):
    ...
    if key == b'a':
        basket_x = max(0, basket_x - basket_speed)  # Don't go left of window
    elif key == b'd':
        basket_x = min(width - basket_width, basket_x + basket_speed)  # Don't go right of window
